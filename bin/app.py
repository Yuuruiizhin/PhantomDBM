...
def create_database():
    ...
    # Validación de seguridad: Asegurar que el nombre de la base de datos solo contenga caracteres válidos
...
def create_table(database_name):
    ...
    # Validación de seguridad para el nombre de la base de datos
    if not all(c.isalnum() or c in '_-' for c in database_name):
        return jsonify({'error': 'Nombre de base de datos inválido.'}), 400
    ...
    try:
        ...
        for col in columns:
            ...
            # Validación de seguridad para los nombres de las columnas
            if not col_name or not all(c.isalnum() or c in '_-' for c in col_name):
                return jsonify({'error': f'Nombre de columna inválido: {col_name}'}), 400
            
            # Validación de seguridad para los tipos de datos
            valid_types = {'VARCHAR', 'INT', 'TEXT', 'DATE', 'DATETIME', 'FLOAT', 'DECIMAL', 'TINYINT', 'SMALLINT', 'MEDIUMINT', 'BIGINT', 'DOUBLE', 'BLOB'}
            if not col_type or col_type.upper() not in valid_types:
                return jsonify({'error': f'Tipo de columna inválido: {col_type}'}), 400

            col_def = f"`{col_name}` {col_type.upper()}"
            if col_length is not None:
                # Validación de que la longitud sea un número entero.
                if not isinstance(col_length, int):
                    return jsonify({'error': 'La longitud de la columna debe ser un número entero.'}), 400
        ...
        return jsonify({'message': f'Tabla "{table_name}" creada correctamente.'}), 201
    except pymysql.MySQLError as err:
        conn.rollback()
...
def delete_table(database_name, table_name):
    ...
    # Validación de seguridad para los nombres
    if not all(c.isalnum() or c in '_-' for c in database_name):
        return jsonify({'error': 'Nombre de base de datos inválido.'}), 400
    if not all(c.isalnum() or c in '_-' for c in table_name):
        return jsonify({'error': 'Nombre de tabla inválido.'}), 400
...
if __name__ == '__main__':
    ...
    app.run(host=local_ip, debug=True, port=4861)